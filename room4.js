...
mobileControls = initMobileControls({
  ...
  onInteract: (raycaster) => {
    ...
    if (nftViewer && nftViewer.isOpen && nftViewer.isOpen()) {
      console.log('[R4 mobile] onInteract called but viewer is open, ignoring');
      return;
    }
    ...
    if (hit.face && nft && nft.matrixWorld) {
      ...
      console.log('[R4 interact] normal alignment', alignment.toFixed(3));
    ...
    if (!isFrontFacing) {
      console.log('[R4 interact] rejected: surface not front-facing enough');
      return;
    }
...
function handleNFTClick(event) {
  ...
  // Log full intersects list for debugging
  console.log(
    '[R4 interact] desktop intersects:',
    intersects.map((hit, i) => ({
      i,
      distance: hit.distance.toFixed(2),
      name: hit.object.name || '(unnamed)',
  ...
  if (hit.face && object && object.matrixWorld) {
    ...
    console.log('[R4 interact] desktop normal alignment', alignment.toFixed(3));
  ...
  if (!isFrontFacing) {
    console.log('[R4 interact] desktop rejected: surface not front-facing enough');
    return;
  }
