...
const mobileControls = initMobileControls({
  ...
  onInteract: (raycaster) => {
    console.log('[Room2 onInteract] Raycasting against', picturePlanes.length, 'picture planes');
    const intersects = raycaster.intersectObjects(picturePlanes, false);
    console.log('[Room2 onInteract] Found', intersects.length, 'intersections');

    if (intersects.length > 0) {
      const nft = intersects[0].object;
      console.log('[Room2 onInteract] First hit:', nft.userData);

      if (nft.userData?.isNFT && typeof nftViewer?.open === 'function') {
        console.log('[Room2 onInteract] Opening NFT viewer for index:', nft.userData.index);
        nftViewer.open(nft.userData.index);  // Pass NFT ID directly
      } else {
        console.log('[Room2 onInteract] Cannot open viewer - isNFT:', nft.userData?.isNFT, 'viewer available:', typeof nftViewer?.open);
      }
    }
  }
});
