...
mobileControls = initMobileControls({
  ...
  onInteract: (raycaster) => {
    ...
    console.log('[R2 interact] mobile tap, performing raycast');
    ...
    if (intersects.length === 0) {
      console.log('[R2 interact] no picturePlanes hit');
      return;
    }

    // Log full intersects list for debugging
    console.log(
      '[R2 interact] intersects:',
    ...
    if (hit.face && nft && nft.matrixWorld) {
      ...
      console.log('[R2 interact] normal alignment', alignment.toFixed(3));
    ...
    if (!isFrontFacing) {
      console.log('[R2 interact] rejected: surface not front-facing enough');
      return;
    }

    console.log('[R2 interact]', {
      distance: hit.distance.toFixed(2),
    ...
    if (!nft.userData?.isNFT) {
      console.log('[R2 interact] hit non-NFT picturePlane, ignoring');
      return;
    }

    if (hit.distance > MAX_NFT_INTERACTION_DISTANCE) {
      console.log('[R2 interact] rejected: too far (distance', hit.distance.toFixed(2), ')');
      return;
    }

    if (nftViewer) {
      console.log('[R2 interact] accepted: opening NFT #', nft.userData.index);
...
function handleNFTClick(event) {
  ...
  if (intersects.length === 0) {
    console.log('[R2 interact] desktop click: no picturePlanes hit');
    return;
  }

  // Log full intersects list for debugging
  console.log(
    '[R2 interact] desktop intersects:',
    intersects.map((hit, i) => ({
      i,
      distance: hit.distance.toFixed(2),
      name: hit.object.name || '(unnamed)',
  ...
  if (hit.face && object && object.matrixWorld) {
    ...
    console.log('[R2 interact] desktop normal alignment', alignment.toFixed(3));
  ...
  if (!isFrontFacing) {
    console.log('[R2 interact] desktop rejected: surface not front-facing enough');
    return;
  }

  console.log('[R2 interact] desktop click:', {
    distance: hit.distance.toFixed(2),
  ...
  if (nftViewer) {
    console.log('[R2 interact] accepted: opening NFT #', object.userData.index);
