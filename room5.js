...
mobileControls = initMobileControls({
  ...
  onInteract: (raycaster) => {
    ...
    if (nftViewer && nftViewer.isOpen && nftViewer.isOpen()) {
      console.log('[R5 mobile] onInteract called but viewer is open, ignoring');
      return;
    }

    console.log('[R5 interact] mobile tap, performing raycast');
    ...
    if (intersects.length === 0) {
      console.log('[R5 interact] no picturePlanes hit');
      return;
    }

    // Log full intersects list for debugging
    console.log(
      '[R5 interact] intersects:',
    ...
    if (hit.face && nft && nft.matrixWorld) {
      ...
      console.log('[R5 interact] normal alignment', alignment.toFixed(3));
    ...
    if (!isFrontFacing) {
      console.log('[R5 interact] rejected: surface not front-facing enough');
      return;
    }

    console.log('[R5 interact]', {
      distance: hit.distance.toFixed(2),
    ...
    if (!nft.userData?.isNFT) {
      console.log('[R5 interact] hit non-NFT picturePlane, ignoring');
      return;
    }

    if (hit.distance > MAX_NFT_INTERACTION_DISTANCE) {
      console.log('[R5 interact] rejected: too far (distance', hit.distance.toFixed(2), ')');
      return;
    }

    if (nftViewer) {
      console.log('[R5 interact] accepted: opening NFT #', nft.userData.index);
...
function handleNFTClick(event) {
  ...
  if (intersects.length === 0) {
    console.log('[R5 interact] desktop click: no picturePlanes hit');
    return;
  }

  // Log full intersects list for debugging
  console.log(
    '[R5 interact] desktop intersects:',
    intersects.map((hit, i) => ({
      i,
      distance: hit.distance.toFixed(2),
      name: hit.object.name || '(unnamed)',
  ...
  if (hit.face && object && object.matrixWorld) {
    ...
    console.log('[R5 interact] desktop normal alignment', alignment.toFixed(3));
  ...
  if (!isFrontFacing) {
    console.log('[R5 interact] desktop rejected: surface not front-facing enough');
    return;
  }

  console.log('[R5 interact] desktop click:', {
    distance: hit.distance.toFixed(2),
  ...
  if (nftViewer) {
    console.log('[R5 interact] accepted: opening NFT #', object.userData.index);
